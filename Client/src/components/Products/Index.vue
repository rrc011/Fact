  ...
  <div>
    <el-card class="box-card">
      ...
      <el-table v-loading="loading" :data="data.items" style="width: 100%">
        ...
        <el-table-column prop="name" label="Nombre de Producto"></el-table-column>
        <el-table-column prop="price" label="Precio"></el-table-column>
        <el-table-column prop="stock" label="Stock"></el-table-column>
        <el-table-column prop="warehouseName" label="Almacen"></el-table-column>
        <el-table-column>
          <template slot-scope="scope">
            <el-button-group size="medium">
              <el-button
                icon="el-icon-edit-outline"
                @click="$router.push(`/product/${scope.row.productId}/edit`)"
              ></el-button>
              <el-button
                icon="el-icon-delete"
                :loading="deleted"
                type="danger"
                @click="remove(scope.row.productId, infoPaginacion.actual)"
              ></el-button>
            </el-button-group>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    remove(id, page) {
      ...
      function _remove(_page) {
        self.delete = true;
        self.$store.state.services.ProductService.delete(id).then(r => {
          self.getAll(_page, "");
          if (r.data) {
            self.$message({
    ...
    next(page, search) {
      ...
      self.$router.push(`/product/page/${parseInt(p) + 1}`);
    },
    prev(page, search) {
      let self = this;
      var p = self.$route.params.page;
    ...
    handleCurrentChange(page, search) {
      ...
      self.$router.push(
        `/product/page/${parseInt(self.infoPaginacion.actual)}`
      );
      this.getAll(page, search);
    },
